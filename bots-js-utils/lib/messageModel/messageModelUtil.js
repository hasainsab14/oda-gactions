...
function textMessageToText(resp) {
  ...
  if (resp.actions && resp.actions.length > 0) {
    result = result + actionsToText(resp.actions, 'Puoi scegliere tra le seguenti opzioni: ');
  }
  if (resp.globalActions && resp.globalActions.length > 0) {
    result = result + actionsToText(resp.globalActions, 'Le seguenti azioni sono disponibili: ');
  }
  return result;
}
...
function cardToText(card, cardPrefix) {
  ...
  if (card.actions && card.actions.length > 0) {
    cardText = cardText + actionsToText(card.actions, 'Le seguenti azioni sono disponibili per questa card: ');
...
function cardsSummaryToText(cards, prompt) {
  var cardsText = prompt || 'Per maggiori informazioni puoi scegliere tra le card seguenti: ';
